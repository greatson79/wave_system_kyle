...
│   ├── agents/
│   │   ├── translator.md            (영→한 번역, glossary 기반, pACS Python-deterministic 측정)
│   │   ├── reviewer.md              (적대적 리뷰어, Enhanced L2)
│   │   ├── fact-checker.md          (사실 검증, claim-by-claim)
│   │   ├── investscan-orchestrator.md  (InvestScan 전용 마스터 오케스트레이터 — advance_step() SOT 가드 포함)
│   │   ├── p1-critical-builder.md   (P1 Critical 모듈 빌더 — Opus 모델, 95% TDD 적용)
│   │   ├── module-builder.md        (TDD-first 모듈 빌더 — 표준 구현)
│   │   ├── tdd-runner.md            (테스트 검증 에이전트 — 전체 테스트 스위트 실행)
│   │   ├── data-collector.md        (FRED/EnvironmentScan 데이터 수집)
│   │   ├── report-reviewer.md       (리포트 품질 리뷰 — narrative_cross_check 포함)
│   │   ├── fred-subagent.md         (FRED API 전용 데이터 수집)
│   │   ├── gnews-subagent.md        (GlobalNews 신호 수집)
│   │   └── envscan-subagent.md      (EnvironmentScan WF1 통합)
│   ├── commands/
...
├── investscan/
│   ├── export_report.py             ← 리포트 TXT+PDF+MD 내보내기 모듈
│   └── ... (27개 모듈)
├── prompt/                          ← 프롬프트 자료
...
InvestScan 파이프라인 완료 후 리포트를 TXT + PDF + MD 형식으로 저장합니다.

| 항목 | 내용 |
|------|------|
| **출력 형식** | TXT (평문) + PDF (fpdf2) + MD (원본 복사) |
| **저장 경로** | `~/Desktop/Ai_works/output/투자분석제안/` |
| **파일명 패턴** | `{YYYY-MM-DD}_주간투자분석.{txt,pdf,md}` |
| **실행 명령** | `python3 -m investscan.export_report` |
| **설계 결정** | ADR-056 참조 |
...
## 스마트 라우터 — 자연어 시작 명령 처리

> **CRITICAL**: 아래 시작 명령 패턴이 감지되면, 반드시 `/start` 커맨드를 실행한다.
> 시작 명령 응답 시 **"Infrastructure Build 재실행"** 옵션을 절대로 표시하지 않는다.

### 시작 명령 감지 패턴

다음 중 하나가 입력되면 즉시 `/start` 로직을 실행한다:

| 감지 패턴 | 예시 |
|----------|------|
| 한국어 시작 의도 | "시작", "시작하자", "시작해", "시작해줘", "시작하겠습니다", "작업 시작", "워크플로우 시작" |
| 영어 시작 의도 | "start", "begin", "let's start", "let's go", "go" |
| 실행 의도 | "실행하자", "실행해줘", "돌려줘", "run", "execute" |
| 투자분석 시작 | "투자분석을 시작하자", "투자 분석 시작", "분석 시작해줘", "투자 시작" → invest-analysis 커맨드 실행 |
| 일반 시작 | "시작", "start", "워크플로우 시작" → start 커맨드 실행 |

### 라우팅 규칙

```
사용자 입력 → 시작 의도 감지 → /start 커맨드 실행
  └→ Step 1: 시스템 상태 자동 진단 (state.yaml 읽기)
  └→ Step 2: 제품 실행 모드 선택 메뉴 표시 (6개 옵션)
  └→ Step 3: 사용자 선택 → 해당 모드 실행
```

### 절대 금지 규칙 (시작 명령 응답 시)

- ❌ "Infrastructure Build 재실행" 옵션 표시
- ❌ "Phase B 재실행" 옵션 표시
- ❌ 빌드·초기화·재구축 관련 옵션 일체
- ✅ 위 항목을 요청받으면: "빌드는 완료 단계입니다. 제품 실행 모드는 6가지 옵션만 지원합니다." 안내
...
| 사용자 요청 패턴 | 스킬 | 진입점 |
|----------------|------|--------|
| "투자분석을 시작하자", "투자 분석 시작", "분석 시작해줘", "투자 시작" | `invest-analysis` | `.claude/commands/invest-analysis.md` |
| "시작", "start", "실행", "워크플로우 시작" 등 | `start` | `.claude/commands/start.md` |
...
5. **P5 — English-First 실행**: 에이전트·스킬·훅 내부 로직은 영어로 작성 (토큰 효율 + 정확도)
6. **P6 — Python-First 판단**: "Python is the judge, LLM is the narrator." 분류·임계값·검증 등 결정론적 판단은 반드시 Python 코드로 수행. LLM은 서사(narration)만 담당. 예: pACS 산술은 `pacs_calculator.py`가, 방향 분류는 `stock_selector.py`가 수행
