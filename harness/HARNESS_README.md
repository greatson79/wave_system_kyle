...
A production-grade, **three-agent harness** for long-running autonomous application
development. GAN-style **Generator ↔ Evaluator** loop, supervised by a **Planner**,
...
`artifacts/`. The app being built lives in an **isolated workspace with its own git
...
│   PLANNER    │  → product vision, design language, AI features, 4–8 sprints
└──────┬───────┘
       │
       ▼  for each sprint:
┌──────────────┐   contract    ┌──────────────┐
...
├── prompts/                   # *_system.md — complete standalone agent system prompts
├── schemas/                   # product_spec / sprint_contract / qa_report JSON Schemas
├── criteria/evaluation_criteria.md   # 4-dimension grading rubric
├── config/
...
python run_harness.py "Build a kanban board with AI task prioritization" --dry-run
...
python run_harness.py "Build a kanban board with AI task prioritization"

# Name the run / workspace, resume from a sprint, use a custom config
python run_harness.py "..." --run-id kanban
python run_harness.py "..." --run-id kanban --start-sprint 3
...
- `harness.max_sprints / max_qa_rounds / qa_pass_threshold / min_criteria_per_sprint`
...
| Agent | cwd | Tools | MCP |
|-------|-----|-------|-----|
| Planner | `harness/` | Read, Write, Edit, Glob, Grep, Skill, WebFetch | — |
...
```bash
python run_harness.py "Create a personal finance tracker with AI spending insights and budget forecasting"
```

Expected: a `PRODUCT_SPEC.md` with ≥4 sprints + design language; contracts with ≥15
criteria each; Evaluator QA driving the live app via Playwright; QA reports with
PASS/FAIL per criterion; at least Sprint 1 completing with a working UI.
