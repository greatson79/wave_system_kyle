...
## 핵심 사실 (내면화)
- 호칭: 사용자를 **"주인님"**이라 부른다.
- **런타임 = cmux 메인**(2026-07-10 역전·cys=보조): 주소 = prefixed ref(`cmux send --workspace workspace:N --surface surface:M` — ★bare 숫자 금지·12a0238 규약)·해소 = 티켓명시 > `tower_roster.json` > `cmux_addr.py` / 부트 = `bash .claude/cmux-adapters/boot_tower.sh`(4종 의무·멱등). cys **보조 세션에서만** 구절차(`cys send --to <역할>`·`cys launch-agent`·`cys boot`) 유효. 위임 = `javis_orchestra.py task-prompt` 의무(런타임 무관).
- **자율주행 위임권 ON**: denylist(soul·CLAUDE.md 변경·외부발행·비가역삭제·로드맵이탈) 밖·가역이면 무정지 자동진행. kill-switch = 주인님 입력 시 즉시 일시정지.
- **계층형 조직**: **관제타워**(경영본부 cmux 워크스페이스 ws1)에 **CEO(총괄·master)+COO+CSO+품질감사 agy(reviewer-gemini)·Codex(reviewer-codex)** 상주 / **8본부 15팀**(2026-07-23 개정·사업부 폐지)은 작업 시 cmux 워크스페이스+본부장(Sub-Master 자율) / 흐름 Worker→본부장→COO→CEO→주인님. **CSO 중앙 1개 고정**(전역 자원 단일권한). **리뷰어 중앙이 전 작업 리뷰**(기본)+본부장 호출권한. **병렬=sub-agent, 전문성=skill 겸용**(pane 남발 금지, 2단 중첩위임 금지). 조직도 정본=`.claude/org/README.md`.
- **쌍방향 통신 대원칙**: 전 노드는 주소로 상호 직접 push 가능한 동등노드. 통신두절=치명결함. 노드 현황 = `cmux tree --all`(+선별 read-screen) 실측(cys 보조 세션은 `cys list`).
- **지침 영속성**: 확장층은 CLAUDE.md @import, 엔진층은 CYS hook — /clear·재시작에도 이중으로 재주입.
- **주인님 직접명령 보고**: 주인님이 개별 본부장에게 직접 명령하면, 그 본부장은 즉시 CEO에 push 보고(cmux prefixed ref로 CEO pane) — 수령·착수·완료. 총괄 전지 유지.
...
- `_round/SESSION_STATE.md`(훅 주입 정본) + `RECOVERY.md`(루트) — 프로젝트 작업기억 + 복구 절차. (루트 구본은 `_archive/` 격하·2026-07-10 / 엔진 세션상태 = `~/.cys/pack/round/SESSION_STATE.md` — 별개·혼동 금지)
...
## 운영 메모리
- 스킬 베이스: 루트 `.claude/skills/` 레지스트리(436개·2026-07-24)(`bash .claude/build_skill_registry.sh` 재구축).
- 자비스 전환·계층 재편 완료: 2026-06-12. **CYS 런타임 이주(헌장 2계층화): 2026-07-04.**
- 구세대 cmux 헌장 원본 보관: `.claude/_legacy-cmux/` (cmux 운영 규율 부속서 — CLAUDE.md 환경선언이 정본 지위 규정).
