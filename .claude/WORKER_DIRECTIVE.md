...
- **★보고 체계 준수 — 워커 직보 금지 (2026-07-18 주인님)**: 본부 산하 워커는 **경영본부(CEO)로
  직보하지 않는다.** 보고 경로 = **본부 워커 → 작업팀장(본부가 세운 경우) → 본부장 → 경영본부 COO**
  (→ CEO → 주인님). 자기 상급자(작업팀장/본부장)에게만 보고하고, 상위 취합은 본부장·COO가 한다.
- **★중복보고 금지 — 본부장도 일상 진행보고는 COO 단일 (2026-07-20 주인님·토큰/컨텍스트 절약)**:
  본부장(팀장) 자신의 **일상 진행보고·단계보고도 COO에게** 올린다(CEO 직접 금지). CEO 직접 push는
  ①주인님 직접명령 수령 통지 ②중대·게이트·에스컬레이션에 한정한다. **CEO와 COO에 같은 보고를
  중복 전송하지 않는다**(중복 수신 = 토큰·컨텍스트 낭비). CEO가 직접 받아도 처리·회신은 COO 라인이
  흡수한다 — 본부장은 진행보고를 COO로 단일화해 관리 체계를 철저히 지키고 혼선을 방지한다.
- **★작업 종료 시 본부장 보고 → 소환해제 (2026-07-18 주인님)**: 본부장은 부서 작업이 종료되면
  **즉시 CEO(팀장)에게 완료 보고**한다(idle 방치 금지·토큰 절약). CEO가 그 ws를 CSO에 전달하면
...
- **★홈페이지(wave-homepage) 소관 = 개발본부**(제작·이미지 포함 — v8~v14 그룹사진 전 버전 개발
  제작·정본/프롬프트계약/패널규약/생성이력 개발 축적). 이미지 제작 시 Codex 워커/Gemini 워커를
  소환(pane 가시화 §5)해 작업한다. 이미지 품질 검수축(레티나 2x 얼굴선명도 등)은 크리에이티브가
  전문성으로 참여(빌더≠리뷰어).
- **★카드뉴스·SNS 포스팅용 이미지 = 크리에이티브 주도(불변)**. 즉 "디자인=크리 전유"가 아니라
  **채널별 소관**(홈페이지=개발 / 카드뉴스·포스팅=크리)이며, 디자인 스킬·지침 자체는 공유.
  [[feedback_design_skills_shared_cross_division]]
