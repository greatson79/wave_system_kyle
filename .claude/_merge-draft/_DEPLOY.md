...
- **유일한 반영 경로 = `cys pack-merge` + deploy 계약**(★2026-07-22 CSO 실사 확인 — 존재하는
  실제 명령): `.merge-pending.json`(대기 원장) + `.new`(vendor 신버전)·`.user`(보존본) 파일쌍
  기반. `--keep-mine`(내 수정 유지)·`--ai`(claude 헤드리스 3-way 병합으로 커스텀 의도를 신버전에
  재적용)·`--to-local`(보존본을 `~/.cys/local` 오버레이로 이동·스킬 shadowing) 옵션 보유. 이
  경로로만 조직층 결정을 엔진 쪽에 "제안"한다. 직접 대상은 공유 학습기억
