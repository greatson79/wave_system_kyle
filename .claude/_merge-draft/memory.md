...
- **주인님 직접명령 보고**: 주인님이 개별 팀장에게 직접 명령하면, 그 팀장은 즉시 CEO에 push 보고(`cys send --queued --to master`) — 수령·착수·완료. 총괄팀장 전지 유지.
- **팀 도구·협업**: 팀장은 리뷰어 socket 호출 또는 자기 부서에 gemini(리뷰)·codex(협업) 소환(쿼터 임계→CSO 감시→추가계정). 크로스팀 자료는 팀↔팀 직접 or CEO 경유, CEO 능동 라우팅(전지 의무).
