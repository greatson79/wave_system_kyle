...
- **★이벤트 구동 소환해제 워크플로우 (2026-07-18 주인님 — CSO 상시 폴링 부담 해소)**: CSO가 본부의
  작업 종료를 상시 확인하기는 어렵다. 따라서 해제는 **본부장 완료 보고 → CEO(총괄)가 해당 본부 ws를
...
> 실측원을 `5h used_pct`로만 명시해 **7d가 95%인데 게이팅이 미발동**하는 취지 반대 결과가
> 났다. 구속력은 **소진 시 손실 크기**로 정해진다 — 5h 소진은 최대 5시간 대기(리셋 여러 회
> 남음)지만 **7d 소진은 리셋까지 전 노드 정지**다. 따라서 두 창을 **OR 조건**으로 함께 본다.
> "5h가 여유하니 괜찮다"는 논증은 성립하지 않는다.

**5h·7d를 항상 둘 다 실측하고, 어느 쪽이든 임계에 닿으면 해당 단계를 발동한다(OR).**
좁은 pane은 상태줄의 7D 구간이 잘려 CW/5H만 보이므로, **7D가 렌더되는 노드에서 직접 확인**
한다 — 잘린 화면을 추정으로 메우지 않는다.

#### 5h 창 (현행 유지 — 주인님 확정)
| 임계 | 조치 |
|---|---|
| **92%+** | 전 노드 무거운 작업 게이팅(계획·문서·검수만) — 업무 평면 집행은 COO |
| **97%+** | 전 노드 완전 graceful-stop + CEO·주인님 즉시 보고. **자동재개 절대금지**(리셋까지 대기) |

#### 7d 창 (신설 3단계 — 주인님 지정 임계 80/90/95)
7d는 리셋까지의 **잔여 시간이 길다**(최대 7일). 따라서 단순 정지가 아니라
**「잔여% ÷ 잔여시간」 시간당 배급** 관점으로 조치를 건다.

| 임계 | 단계 | 조치 |
|---|---|---|
| **80%+** | 주의 | ①모델 티어 상향 자제(자율조정권 §3 상향 기준을 엄격 적용) ②**서브에이전트 팬아웃 억제**(노드당 동시 2개 상한) ③재생성·재작업 금지 ④CEO·COO에 통지, 잔여시간 대비 배급률 산출 |
| **90%+** | 위험 | ①무거운 작업 게이팅(5h 92%와 동급) ②**상주 노드 티어 하향**(COO·CSO → Sonnet) ③우선순위 밖 트랙 보류 ④신규 본부 소환 억제 ⑤코드성 작업은 codex, 검수는 agy로 분산 |
| **95%+** | 임계 | ①**신규 착수 전면 금지** ②진행 중 작업은 **저장 가능한 단위까지만** 완료 후 정지 ③**최우선순위 트랙만 계속** ④CEO·주인님 즉시 보고 ⑤**자동재개 금지** — 재개는 주인님/CEO 명시 지시로만 |

- **단계는 하향 이력이 아니라 현재 실측값으로 판정**한다(한 번 발동했다고 고정되지 않음).
- **두 창의 단계가 다르면 강한 쪽을 따른다**(예: 5h 15%·7d 95% → 95% 임계 조치 적용).
- 게이팅 발동·해제는 **CSO가 선언**하고 **COO가 업무 평면에서 집행**한다(COO v3.1 §2 연동).
...
- **★전달 유실 3규율 (2026-07-27 신설 · 정본 [[feedback_cmux_send_multiline_fragmentation]])**:
  ①**개행 없는 단일 줄로만** 전송(개행 멀티라인은 첫 개행에서 제출돼 **조용히 분절·유실**)
