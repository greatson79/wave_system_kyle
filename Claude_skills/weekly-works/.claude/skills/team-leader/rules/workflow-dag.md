...
| D. 소그룹 나눔지 | A 4.5단계 (제목 확정) | E,H와 병렬 | Small Group Agent |
| E. SNS 카드뉴스 | A 4.5단계 (제목 확정) | D,H와 병렬 | SNS Card News |
| H. 주보 | A 4.5단계 (제목 확정) + 광고 사용자 입력 | D,E와 병렬 | Bulletin Agent |
...
Phase 1-Interactive (메인 대화):
  A (설교) — 대화형, 목사 피드백 필수
  1단계 → 2단계 → 3단계 → 4단계 구조설계
  → 설교 중 백그라운드 완료 알림이 도착할 수 있음

[게이트 1] 4.5단계 제목 확정 → sermon-context.md 갱신

Phase 2 (백그라운드 — 4.5단계 직후 즉시, 자동 병렬 소환):
