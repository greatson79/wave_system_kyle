...
> **통합 포인트**: 신학적 주제 + 문학적 장치를 종합한 후, 우상 분석을 대화에서 진행.
> 우상은 4개 범주로 고정되지 않는다 — "이 시대의 삶의 자리에서 하나님을 대체하는 모든 것"이 우상이다.
> Keller의 권력/승인/안전/쾌락은 참고 도구일 뿐이며, 본문이 요구하는 우상 범주를 우선 도출한다.
...
> ⚠️ **핵심 원칙**: Research Bridge는 자동 실행이다. 사용 여부를 목사에게 묻지 않는다.
> 목사에게 묻는 것은 **에이전트 범위** 한 가지뿐이다.
...
1단계 상황파악 완료 후:
  ...
  [목사에게 1회만 확인] →
    "이 본문은 {장르}입니다.
     추천 에이전트: {추천 에이전트 목록}
     전체 11개로 진행할까요, 추천 세트로 진행할까요?"
  [목사 선택 즉시 → 자동 실행. 추가 허락 불필요]
```
...
> ⚠️ **절대 규칙**: 각 2-N 단계는 반드시 아래 순서를 지킨다.
> **에이전트 실행 → 결과 제시 → 목사와 대화 → 산출물 확정 → STOP**
> 목사의 명시적 "다음으로" 없이 다음 단계로 자동 진행하지 않는다.

```
1단계 상황파악 완료
  ↓
  [에이전트 범위 1회 확인] "전체 11개 vs 추천 세트?"
  ↓ (목사 선택)
  ════════════════════════════════════════
  [2-1 단계]
  ① 에이전트 병렬 소환 (background)
     Agent(@original-text-analyst)
     Agent(@manuscript-comparator)
     Agent(@keyword-expert)
  ② 결과 수신 → 목사에게 요약 제시
  ③ 목사와 대화 (통찰 심화, 방향 조율)
  ④ 산출물 2-1_원어분석.md 확정 저장
  ⑤ STOP → 목사가 "다음" 또는 "계속"할 때만 진행
  ════════════════════════════════════════
  [2-2 단계]
  ① 에이전트 병렬 소환
  ...
  ② 결과 수신 → 요약 제시 (+ Keller 우상 프레임워크 적용)
  ③ 목사와 대화
  ④ 산출물 2-3_우상분석.md 확정 저장
  ⑤ STOP
  ════════════════════════════════════════
  [2-4 단계]
  ① 에이전트 병렬 소환
  ...
  ③ 결과 수신 → 요약 제시
  ④ 목사와 대화 (CMT · FCF · HP 도출)
  ⑤ 산출물 2-4_종합통찰.md 확정 저장
  ⑤ STOP → 2단계 전체 완료 보고
  ════════════════════════════════════════
  3~5단계: 각 단계 완료 후 동일하게 STOP, 목사 확인 후 진행
```
