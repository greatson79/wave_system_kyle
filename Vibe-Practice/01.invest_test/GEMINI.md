...
| Translation pACS (Python-First) | P6 원칙 적용: 번역 후 `pacs_calculator.py`가 실제 텍스트에서 Ft/Ct/Nt를 결정론적으로 산출. **Python 점수가 권위적(authoritative)**. AI 자기보고는 사전 자기검사(pre-mortem)에만 유효. PA8 3-tier: FAIL(Python pACS<50 — 시스템 RED), WARN(50-69 — 품질 RED), 괴리 확인(≥70 — AI vs Python delta>15 경고). 번역 품질 기준: GREEN≥85, YELLOW 70-84, RED<70. `pacs_calculator.py --source --target` CLI 사용 |
...
- **P5**: 에이전트·스킬·훅 내부 로직은 영어로 작성 (토큰 효율 2-3배 + 정확도 향상)
- **P6**: "Python is the judge, LLM is the narrator." 분류·검증·임계값 판단은 Python 코드. LLM은 서사만. 예: pACS 산술은 Python, 방향 분류는 stock_selector.py
