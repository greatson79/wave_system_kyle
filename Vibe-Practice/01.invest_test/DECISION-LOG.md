...
### ADR-051: Claude Code v2.1 신기능 연구 (기 기록)

- **날짜**: 2026-03-02
- **상태**: Accepted
- (기존 항목 — 타임라인 일치화를 위해 섹션 명시)

---

### ADR-052: P6 Python-First 원칙 — pACS 산술 결정론화 + advance_step() 단조 SOT 가드

- **날짜**: 2026-03-29
- **상태**: Accepted
- **맥락**: InvestScan 전수조사에서 3개의 P1 취약점 발견. (1) `pacs_calculator.py`가 CLI 진입점 없어 Orchestrator가 Python 점수를 자동 취득 불가 → AI 자기보고에만 의존. (2) `validate_pacs.py`의 PA8 로직이 pACS 50-69 구간(grade="RED")에서 경고를 생략하는 Silent RED Gap. (3) SOT 단계 전진이 Orchestrator가 `atomic_sot_write()`를 수동 호출해야 하는 구조로 bypass 가능.
- **결정**:
  1. `pacs_calculator.py`에 `__main__` CLI 블록 추가. Exit code: 0 = GREEN/YELLOW (pACS≥70), 1 = RED (grade=="RED", pACS<70). `--source`, `--target`, `--glossary` 인자 지원
  2. `validate_pacs.py`에 PA8 3-tier 로직 구현: Tier 1 FAIL(pACS<50 — 시스템 RED, 차단), Tier 2 WARN(50-69 — 품질 RED, 로그만), Tier 3 괴리 확인(≥70 — |AI-Python|>15 경고)
  3. `investscan-orchestrator.md`에서 `atomic_sot_write()` + 별도 검증 패턴을 `advance_step()` 단일 함수로 대체. `advance_step()`은 `new_step == current_step + 1` 단조 검증을 내장 — 건너뜀/되감기 불가
  4. 번역 품질 기준 통일: GREEN≥85, YELLOW 70-84, RED<70 (번역 pACS는 에이전트 자기평가 pACS보다 엄격)
- **근거**: "Python is the judge, LLM is the narrator." pACS 산술이 LLM 자기보고에 의존하면 PA8 경고가 훈련 데이터에 포함되어 점수를 부풀리는 incentive가 발생. Python 코드가 실제 텍스트를 측정해야만 이 인센티브를 제거할 수 있다. advance_step()은 SOT 단조성 위반을 사전에 차단하여 다중 에이전트 환경에서 단계 역행을 원천 봉쇄.
- **대안**: (1) AI 자기보고 유지 + 주기적 감사 → 기각 (감사 누락 가능, PA8 Silent RED Gap 구조적 잔존). (2) 별도 validate_sot_step() 함수 → 기각 (Orchestrator가 호출 누락 시 bypass 가능)
- **관련 파일**: `investscan/pacs_calculator.py`, `.claude/hooks/scripts/validate_pacs.py`, `.claude/agents/investscan-orchestrator.md`, `.claude/agents/translator.md`

---

### ADR-053: Schema SOT 완결성 — SteepsCategory StrEnum, SectorDirection 타입화, ContextContract 인터페이스

- **날짜**: 2026-03-29
- **상태**: Accepted
- **맥락**: InvestScan 전수조사에서 4개의 Schema SOT 위반 발견. (1) `SteepsCategory`가 일반 상수로 분산 정의 — lowercase 's' (security) vs uppercase 'S' (social) 혼동 위험. (2) `SectorDirection`이 plain dict로 전달되어 타입 계약 없음. (3) `ContextContract`(Stage 1→Stage 2 인터페이스)가 schema.py SOT에 없음. (4) `InvestmentMeta`에 `sectors` 필드 없어 Step 10에서 AttributeError 발생.
- **결정**:
  1. `schema.py`에 `SteepsCategory(StrEnum)` 추가: 6개 값 (T/E/P/S/E_env/s). **lowercase s가 uppercase S와 구별되는 것이 핵심** — Python 타입 시스템이 이를 강제
  2. `schema.py`에 `SectorDirection(frozen=True, slots=True)` 추가: sector_name, direction (Bullish|Neutral|Bearish), confidence, signal_count, signal_ids
  3. `schema.py`에 `ContextContract` 추가 (frozen=False — schema_version에 기본값 필요)
  4. `InvestmentMeta`에 backward-compatible 필드 추가: sectors(list), top_signals, macro_summary, action_item, action_checklist
  5. `synthesize_macro.py`에 P6 rule-based 생성기 4개 추가: `_build_sectors_list()`, `_build_action_item()`, `_build_action_checklist()`, `_build_macro_summary()`
  6. `stock_selector.py`에 `select_stocks(investment_meta, watchlist_override)` 추가 — `classify_category()` 보존으로 backward compatibility 유지
- **근거**: schema.py가 SOT가 되려면 모든 타입 계약이 여기에 있어야 한다. 분산된 타입 정의는 다중 에이전트 환경에서 버전 불일치를 유발한다. StrEnum은 lowercase/uppercase 구분을 런타임이 아닌 컴파일 타임에 강제한다. Backward-compatible 확장으로 기존 코드의 무결성을 보존한다.
- **대안**: (1) plain dict 유지 → 기각 (타입 계약 없음, AttributeError 런타임 크래시). (2) 비-backward-compatible 리네임 → 기각 (10+ 파일 cascading 변경으로 안정성 위협)
- **검증**: `python3 -m pytest tests/ → 718/718 PASS` (신규 필드 추가 후에도 기존 테스트 무결)
- **관련 파일**: `investscan/schema.py`, `investscan/synthesize_macro.py`, `investscan/stock_selector.py`

---

### ADR-054: 파이프라인 통합 테스트 — 9단계 전체 체인 검증

- **날짜**: 2026-03-29
- **상태**: Accepted
- **맥락**: 유닛 테스트 718개가 모듈별 로직을 검증하지만, 스크립트 간 데이터 흐름 계약(schema contract)을 검증하는 통합 테스트가 없었다. `FRED fixture → NarrativeOutput → Markdown → Telegram`까지의 end-to-end 체인에서 한 단계의 출력이 다음 단계의 입력 계약을 만족하는지 자동 검증이 불가했다.
- **결정**: `tests/test_pipeline_integration.py` 신규 작성 (57개 테스트, 9단계 체인):
  - Stage 1: FRED fixture → InvestmentMeta (literal 필드, SectorDirection 타입, sentinel 부재)
  - Stage 2: InvestmentMeta → (cat_a, cat_b) (최대 5/3, watchlist override, get_direction 임계값)
  - Stage 3: stock_code → StockFinancials (dry-run 외부 API 미호출)
  - Stage 4: context_data → NarrativeOutput (sentiment_weight=0.0, ≥1000B, direction 리터럴)
  - Stage 5: NarrativeOutput → python_validate_first + compliance_filter
  - Stage 6: NarrativeOutput → generate_report → Markdown (stock_code/direction/YoY 포함)
  - Stage 7: NarrativeOutput → build_5line_summary → 5-line + send_message(dry-run)
  - Stage 8: EnvScan fixture → normalize_envscan → UnifiedSignal (STEEPS 6카테고리)
  - Stage 9: 전체 E2E + schema SOT 중복 없음 + pACS 계산 + PredictionRecord
- **근거**: 통합 테스트는 유닛 테스트가 잡지 못하는 "단계 간 계약 위반"을 탐지한다. mock 사용을 최소화하고 실제 fixture 데이터로 실제 함수 체인을 실행하면 미래의 API 변경이 즉시 통합 테스트 실패로 나타난다.
- **대안**: (1) E2E 테스트 없이 유닛 테스트만 → 기각 (단계 간 계약 위반은 유닛 테스트로 탐지 불가). (2) Playwright/Selenium → 기각 (웹서버 없음, 정적 HTML은 pytest로 충분)
- **검증**: `python3 -m pytest tests/ → 775/775 PASS` (기존 718 + 신규 57)
- **관련 파일**: `tests/test_pipeline_integration.py` (신규), `tests/fixtures/` (기존 fixture 재활용)

---

### ADR-055: config/few_shot.json v2.0.0 — 4-예제 통합 포맷 + Bearish/Neutral 추가

- **날짜**: 2026-03-29
- **상태**: Accepted
- **맥락**: `config/few_shot.json` v1.0.0은 `{category_a_examples: [...], category_b_examples: [...]}` 분리 구조였으며 Cat A Bullish(삼성전자)와 Cat B Bullish(NAVER) 2개만 존재. `intelligence_engine.py`가 Bearish/Neutral 방향의 few-shot 학습 없이 LLM을 호출하여 이 방향에서 할루시네이션 위험이 높았다. workflow.md Step 5는 4개 예제(Cat-A Bullish/Bearish, Cat-B Bullish, Cat-A Neutral)를 명시적으로 요구.
- **결정**:
  1. 구조를 `{examples: [example_id, category, direction, sector, input_context, expected_output, direction_label]}` 통합 포맷으로 재설계
  2. 버전 1.0.0 → 2.0.0
  3. 4개 완성 예제:
     - Ex 1: Cat A · Bullish · 반도체 — SK Hynix (HBM3E, +41.2% YoY, 외국인 +$520M)
     - Ex 2: Cat A · Bearish · 바이오 — Samsung Biologics (FDA 검사 위험, 34.2% 프리미엄)
     - Ex 3: Cat B · Bullish · AI에이전트 — Kakao (KakaoWork, 34% CAGR, 47M DAU)
     - Ex 4: Cat A · Neutral · IT서비스 — NAVER (Webtoon -$38M EBIT, 8.9% 프리미엄)
- **근거**: LLM few-shot learning의 효과는 예제의 다양성에 비례한다. Bullish 편향 예제만 제공하면 LLM이 항상 긍정 서사를 생성하는 bias가 생긴다. 모든 3개 방향(Bullish/Bearish/Neutral)과 2개 카테고리(A/B)를 커버해야 intelligence_engine이 균형 잡힌 서사를 생성할 수 있다.
- **대안**: (1) 예제를 방향별 파일로 분리 → 기각 (로딩 복잡도 증가). (2) 예제 없이 시스템 프롬프트만으로 → 기각 (few-shot은 LLM 품질 향상의 가장 검증된 방법)
- **관련 파일**: `config/few_shot.json`, `investscan/intelligence_engine.py`

### ADR-056: InvestScan 리포트 출력 형식 — TXT + PDF + MD 3종 + `export_report.py` 모듈

- **날짜**: 2026-03-30
- **상태**: Accepted
- **맥락**: InvestScan 파이프라인은 `.md` 리포트만 생성했다. 사용자가 투자 분석 결과를 실용적으로 활용하려면 (a) 터미널에서 바로 읽을 수 있는 평문 텍스트, (b) 인쇄·공유 가능한 PDF가 필요했다. 저장 경로도 `output/reports/` 내부가 아니라 사용자 Desktop의 전용 폴더로 분리해야 했다.
- **결정**:
  1. 출력 형식: **TXT + PDF + MD 3종** (기본값: txt,pdf; md는 항상 함께 저장)
  2. 저장 경로: `~/Desktop/Ai_works/output/투자분석제안/{YYYY-MM-DD}_주간투자분석.{txt,pdf,md}`
  3. 구현 모듈: `investscan/export_report.py` — `export()` 함수 + CLI entrypoint
  4. PDF 엔진: `fpdf2` (fpdf2 라이브러리; built-in Helvetica 폰트 → latin-1 인코딩 + `_safe()` 변환 레이어)
  5. 날짜 추출: 파일명에서 `YYYY-MM-DD` 정규식으로만 추출 (한글 포함 전체 stem 사용 금지)
- **근거**:
  - TXT: 추가 의존성 없이 UTF-8 텍스트 저장 → 가장 가볍고 이식성 높음
  - PDF: fpdf2는 순수 Python, 외부 바이너리 불필요 → 설치·배포 단순화. WeasyPrint/ReportLab 대비 의존성 최소
  - MD 복사: 원본 소스 참조 편의 + 날짜 기반 파일명 통일
  - `~/Desktop/Ai_works/output/투자분석제안/`: 파이프라인 내부 경로(`output/reports/`)와 분리 → "사용자 산출물 폴더"와 "시스템 중간 산출물 폴더" 역할 구분
- **대안**:
  - WeasyPrint + HTML 렌더링 → 기각 (시스템 의존성 다수: cairo, pango, libffi 등)
  - Pandoc CLI 호출 → 기각 (외부 바이너리 의존 + 설치 안정성 불확실)
  - Word(.docx) 포맷 → 기각 (python-docx 추가, 수신자 편집 가능 → 보고서 무결성 훼손)
- **관련 파일**: `investscan/export_report.py`, `output/투자분석제안/`
...
| 2026-03-29 | accepted | ADR-052: P6 Python-First — pACS CLI + advance_step() 단조 SOT 가드 + PA8 3-tier |
| 2026-03-29 | accepted | ADR-053: Schema SOT 완결성 — SteepsCategory StrEnum + SectorDirection 타입화 + ContextContract |
| 2026-03-29 | accepted | ADR-054: 파이프라인 통합 테스트 — test_pipeline_integration.py 9단계 체인 (775개 테스트) |
| 2026-03-29 | accepted | ADR-055: few_shot.json v2.0.0 — 4-예제 통합 포맷 (Bullish/Bearish/Neutral + Cat B) |
| 2026-03-30 | accepted | ADR-056: InvestScan 리포트 출력 형식 — TXT + PDF + MD 3종 + `export_report.py` 모듈 |
