...
def main():
    ...
    # InvestScan Phase B: capture agent workspace + pacs-logs paths for RLM chain
    # §8 MR-3: minimal extension — append as sot_content suffix (no generate_snapshot_md change)
    import pathlib
    workspace_dir = pathlib.Path(project_dir) / ".claude" / "agent-workspace"
    pacs_dir = pathlib.Path(project_dir) / "pacs-logs"
    investscan_hint = ""
    if workspace_dir.exists():
        ws_files = sorted(workspace_dir.glob("*.yaml"))
        if ws_files:
            investscan_hint += "\n[SubAgent Workspace Files]\n"
            investscan_hint += "\n".join(f"  - {p.name}" for p in ws_files)
    if pacs_dir.exists():
        pacs_files = sorted(pacs_dir.glob("*.md"))
        if pacs_files:
            investscan_hint += "\n[Translation pACS Logs]\n"
            investscan_hint += "\n".join(f"  - {p.name}" for p in pacs_files)
    if investscan_hint:
        sot_content = (sot_content or "") + "\n" + investscan_hint.strip()
