...
E5_RICH_CONTENT_MARKER = "### Modified Files (In Progress)"
...
def check_ulw_compliance(entries):
    ...
    if compliance["errors_detected"] > 0 and compliance["post_error_actions"] == 0:
        compliance["warnings"].append(
            "ULW_NO_SISYPHUS: {} errors detected, 0 post-error actions — I-1 Sisyphus Persistence non-compliant".format(
    ...
    if compliance["task_creates"] == 0 and compliance["total_tool_uses"] >= 5:
        compliance["warnings"].append(
            "ULW_NO_DECOMPOSITION: {} tool uses, 0 TaskCreate — I-2 Mandatory Task Decomposition non-compliant".format(
    ...
    if compliance["task_creates"] > 0 and compliance["task_updates"] == 0:
        compliance["warnings"].append(
            "ULW_NO_PROGRESS: {} TaskCreate, 0 TaskUpdate — I-2 Progress Tracking non-compliant".format(
    ...
    if compliance["task_creates"] > 0 and compliance["task_lists"] == 0:
        compliance["warnings"].append(
            "ULW_NO_VERIFY: {} TaskCreate, 0 TaskList — I-2 completion verification not performed".format(
                compliance["task_creates"]
            )
        )
    ...
    if compliance["max_consecutive_retries"] > 3:
        compliance["warnings"].append(
            "ULW_RETRY_EXCEEDED: {} consecutive retries on same target — I-3 Bounded Retry exceeded (max 3)".format(
...
def generate_snapshot_md(session_id, trigger, project_dir, entries, work_log=None, sot_content=None):
    ...
    sections.append("<!-- IMMORTAL: User task instruction — core context for session recovery -->")
    ...
    if next_step:
        ...
        sections.append("<!-- IMMORTAL: Cognitive continuity anchor for session recovery — next action directive -->")
        sections.append(next_step)
        sections.append("")

    # Section 2: SOT State (deterministic file read)
    sections.append("## SOT State (Workflow State)")
    if sot_content:
        sections.append(f"File: `{sot_content['path']}`")
        sections.append(f"Modified: {sot_content['mtime']}")
        sections.append("```yaml")
        sections.append(sot_content["content"])
        sections.append("```")
    else:
        sections.append("SOT file not found (state.yaml/state.json missing)")
    sections.append("")
    ...
    try:
        ...
        if ap_state:
            ...
            sections.append("<!-- IMMORTAL: Must be preserved on session recovery -->")
            sections.append("")
            ...
            outputs = ap_state.get("outputs", {})
            if outputs:
                sections.append("### Per-Step Output Validation (Anti-Skip Guard)")
    ...
    try:
        ...
        if gate_lines:
            ...
            sections.append(
                "<!-- IMMORTAL: Verification/pACS/Review resume context for session recovery -->"
            )
            sections.append("")
    ...
    try:
        ...
        if team_state:
            ...
            sections.append("<!-- IMMORTAL: Must be preserved on session recovery — RLM Layer 2 -->")
            sections.append("")
            ...
            if pending:
                ...
                sections.append(f"- **⚠ RECOVERY ACTION**: Resume these pending tasks before starting new work")
            sections.append("")

            # Completed summaries (RLM Layer 2 — team work summaries)
            summaries = team_state.get("completed_summaries", {})
            if summaries:
                sections.append("### Teammate Work Summaries (RLM Layer 2)")
    ...
    try:
        ...
        if ulw_state:
            ...
            sections.append(f"- **Source instruction**: {_truncate(ulw_state['source_message'], 500)}")

            # Show Autopilot combination state
            ap_state = read_autopilot_state(project_dir)
            if ap_state:
                sections.append(f"- **Autopilot combined**: Yes (ULW reinforces Autopilot — retry limit 10→15)")
            else:
                sections.append(f"- **Autopilot combined**: No (interactive + ULW)")
            sections.append("")

            sections.append("### ULW Intensifiers")
            sections.append("1. **I-1. Sisyphus Persistence**: Max 3 retries, each attempt using a different approach. 100% completion or report reason for impossibility")
            sections.append("2. **I-2. Mandatory Task Decomposition**: TaskCreate → TaskUpdate → TaskList required")
            sections.append("3. **I-3. Bounded Retry Escalation**: No more than 3 retries on same target — escalate to user if exceeded")
            sections.append("")
            ...
            if ulw_compliance:
                ...
                sections.append(f"- TaskCreate: {ulw_compliance['task_creates']}x")
                sections.append(f"- TaskUpdate: {ulw_compliance['task_updates']}x")
                sections.append(f"- TaskList: {ulw_compliance['task_lists']}x")
                sections.append(f"- Total tool uses: {ulw_compliance['total_tool_uses']}x")
                if ulw_compliance["errors_detected"] > 0:
                    sections.append(f"- Errors detected: {ulw_compliance['errors_detected']}")
                    sections.append(f"- Post-error actions: {ulw_compliance['post_error_actions']}")
                if ulw_compliance["max_consecutive_retries"] > 0:
                    sections.append(f"- Max consecutive retries: {ulw_compliance['max_consecutive_retries']}")
                warnings = ulw_compliance.get("warnings", [])
                if warnings:
                    sections.append("")
                    sections.append("**⚠ Intensifier rule violation detected:**")
                    for w in warnings:
                        sections.append(f"- {w}")
                else:
                    sections.append("")
                    sections.append("✅ All Intensifier rules compliant")
                sections.append("")
    except Exception:
    ...
    try:
        ...
        if os.path.isdir(diag_dir):
            ...
            if diag_files:
                ...
                sections.append("<!-- IMMORTAL: Preserve diagnosis context at session boundaries -->")
                sections.append("")
    ...
    if decisions:
        sections.append(E5_DESIGN_DECISIONS_MARKER)
        sections.append("<!-- IMMORTAL: Preserve 'why' design decisions were made for session recovery -->")
        sections.append("")
    ...
    sections.append(E5_COMPLETION_STATE_MARKER)
    sections.append("<!-- Python deterministic generation — direct reference, no AI interpretation needed -->")
    sections.append("")
    cs = completion_state
    sections.append("### Tool call results")
    ...
    for tk in ["Edit", "Write", "Bash", "Read", "Task", "Grep", "Glob"]:
        ...
        if count > 0:
            if tk == "Edit":
                sections.append(
                    f"- Edit: {count}x called → {cs['edit_success']} success, {cs['edit_fail']} fail"
                )
            elif tk == "Write":
                sections.append(
                    f"- Write: {count}x called → {cs['write_success']} success, {cs['write_fail']} fail"
                )
            elif tk == "Bash":
                sections.append(
                    f"- Bash: {count}x called → {cs['bash_success']} success, {cs['bash_fail']} fail"
                )
            else:
                sections.append(f"- {tk}: {count}x called")
    ...
    for name, count in sorted(other_tools.items()):
        sections.append(f"- {name}: {count}x called")
    sections.append("")

    if cs["file_verification"]:
        sections.append("### File State Verification (at save time)")
        sections.append("| File | Exists | Last modified |")
        sections.append("|------|--------|--------------|")
    ...
    if file_ops:
        ...
        sections.append("(no file modification records)")
    sections.append("")
    ...
    if read_ops:
        ...
        sections.append("(no file reference records)")
    sections.append("")

    # Section 8: User Messages (verbatim — last N)
    sections.append("## User Message History")
    ...
    sections.append("## Claude Key Responses")
    meaningful_texts = [
...
def _compress_snapshot(full_md, sections):
    ...
    prev_size = len(result)
    compressed = _remove_section(compressed, "## Session Statistics")
    result = "\n".join(compressed)
