...
def _extract_brief_summary(content):
    ...
    for line in lines:
        ...
        if line.startswith("## Current Task"):
            ...
            continue
        elif line.startswith("## Session Statistics"):
...
def _build_recovery_output(source, latest_path, summary, sot_warning, snapshot_age, fallback_note="", project_dir=None, snapshot_content="", risk_data=None):
    ...
    # InvestScan Phase B: Translation restore hint (§8 restore_context extension)
    # Surface pacs-logs and agent-workspace paths if they exist
    if project_dir:
        import pathlib
        workspace_dir = pathlib.Path(project_dir) / ".claude" / "agent-workspace"
        pacs_dir = pathlib.Path(project_dir) / "pacs-logs"
        ws_files = sorted(workspace_dir.glob("*.yaml")) if workspace_dir.exists() else []
        pacs_files = sorted(pacs_dir.glob("*.md")) if pacs_dir.exists() else []
        if ws_files or pacs_files:
            output_lines.append("")
            output_lines.append("■ InvestScan 복원 포인터:")
            if ws_files:
                output_lines.append("  SubAgent Workspaces: " + ", ".join(f.name for f in ws_files[:5]))
            if pacs_files:
                output_lines.append("  Translation pACS Logs: " + ", ".join(f.name for f in pacs_files[:5]))
