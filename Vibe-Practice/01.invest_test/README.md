...
## 🧬 현재 활성 자식 시스템 — InvestScan

> 이 코드베이스에서 분화한 자식 시스템입니다.
> InvestScan은 부모의 전체 DNA를 내장하며 독립적으로 운영됩니다.

**InvestScan**: 자동 주간 투자 인텔리전스 시스템
- 글로벌 경제 데이터(FRED, EnvironmentScan) → AI 분석 → TXT + PDF + Telegram 리포트 자동 발송
- 27개 Python 모듈 | 775개 테스트 | M0.5 마일스톤 ✅ 달성
- 출력 형식: TXT + PDF + MD → `~/Desktop/Ai_works/output/투자분석제안/{DATE}_주간투자분석.{txt,pdf,md}`

**빠른 시작 (30초)**:
```
시작하자
```
Claude Code 터미널에 위 한 마디를 입력하면 스마트 라우터가 6가지 실행 모드를 안내합니다.

### InvestScan 문서

| 문서 | 목적 |
|------|------|
| **[INVESTSCAN-README.md](INVESTSCAN-README.md)** | InvestScan 개요 + 빠른 시작 |
| **[INVESTSCAN-USER-MANUAL.md](INVESTSCAN-USER-MANUAL.md)** | 사용자 매뉴얼 ("시작하자" 중심) |
| **[INVESTSCAN-ARCHITECTURE-AND-PHILOSOPHY.md](INVESTSCAN-ARCHITECTURE-AND-PHILOSOPHY.md)** | 아키텍처 + 10단계 파이프라인 + 설계 원칙 |
| **[INVESTSCAN.md](INVESTSCAN.md)** | AI 에이전트 온보딩 문서 |

> **부모-자식 문서 분리 패턴**: `AGENTICWORKFLOW-*.md`는 방법론/프레임워크(부모), `INVESTSCAN-*.md`는 도메인 고유 아키텍처(자식).
> 자식 문서만으로 InvestScan을 이해하고 운영할 수 있습니다.
...
```
AgenticWorkflow + InvestScan 자식 시스템/
│
│  ── InvestScan 자식 시스템 문서 ────────────────────────────────
├── INVESTSCAN-README.md                 ← InvestScan 개요 + 빠른 시작
├── INVESTSCAN-USER-MANUAL.md            ← 사용자 매뉴얼 ("시작하자" 중심)
├── INVESTSCAN-ARCHITECTURE-AND-PHILOSOPHY.md  ← 아키텍처 + 설계 철학
...
├── DECISION-LOG.md                      ← 설계 결정 이력 (ADR, 55개+)
├── soul.md                              ← DNA 유전 철학
│
│  ── InvestScan Python 시스템 ─────────────────────────────────────
├── investscan/                          ← 27개 Python 모듈
│   ├── weekly_orchestrator.py           ← 10단계 파이프라인 오케스트레이터
│   ├── schema.py                        ← 데이터 스키마 SOT (frozen dataclass)
│   ├── synthesize_macro.py             ← FRED 매크로 분석 (P6 Python-First)
│   ├── intelligence_engine.py          ← LLM 내러티브 생성
│   ├── compliance_filter.py            ← 10패턴 법적 표현 검사
│   ├── report_generator.py             ← Jinja2 마크다운 리포트
│   ├── telegram_notifier.py            ← Telegram 한국어 5줄 요약 발송
│   ├── run_summary.py                  ← 실행 결과 3줄 요약
│   ├── preview_report.py               ← 리포트 인라인 미리보기
│   ├── approve_hitl.py                 ← HITL-3 간소화 승인
│   └── ... (17개 추가 모듈)
├── tests/                               ← 26개 테스트 파일 (775개 테스트)
├── output/reports/                      ← 생성된 리포트 (영어+한국어)
│
│  ── Claude Code 인프라 ──────────────────────────────────────────
├── .claude/
...
### InvestScan 자식 시스템 커맨드

| 커맨드 | 기능 |
|--------|------|
| `시작하자` (또는 `start`) | **스마트 라우터 진입** — 상태 진단 + 6가지 실행 모드 선택 |
| `/weekly-report` | 주간 리포트 전체 파이프라인 |
| `/run-investscan` | 데이터 수집 + 분석만 |
| `/approve-hitl 3` | 최종 리포트 승인 + Telegram 발송 |
| `/check-sot` | 시스템 상태 상세 확인 |
| `/run-tdd` | 전체 775개 테스트 실행 |
...
> **InvestScan 자식 시스템**: 이 프레임워크에서 생성된 실제 동작 시스템. 27개 Python 모듈 + 26개 테스트 파일(775개 테스트) + 16개 마일스톤 게이트(M0.5 8개/M1 8개) + 12개 전용 에이전트 정의. P6 Python-First 원칙 적용: `pacs_calculator.py` (번역 품질 결정론적 산술), `advance_step()` (SOT 단조 가드), `stock_selector.py` (카테고리 분류 임계값 기반). 전체 파이프라인 dry-run: `python3 run_m05.py --dry-run && python3 run_m1.py --dry-run`
...
### InvestScan 사용자 (빠른 시작 우선)

| 순서 | 문서 | 목적 |
|------|------|------|
| 1 | **[INVESTSCAN-README.md](INVESTSCAN-README.md)** | InvestScan 개요 + 빠른 시작 ("시작하자") |
| 2 | **[INVESTSCAN-USER-MANUAL.md](INVESTSCAN-USER-MANUAL.md)** | 상세 사용 방법 + FAQ |
| 3 | **[INVESTSCAN-ARCHITECTURE-AND-PHILOSOPHY.md](INVESTSCAN-ARCHITECTURE-AND-PHILOSOPHY.md)** | 10단계 파이프라인 + 모듈 구조 이해 |
| 4 | **[DECISION-LOG.md](DECISION-LOG.md)** | 설계 결정의 맥락과 근거 추적 |

### AgenticWorkflow 프레임워크 이해 (심화)

| 순서 | 문서 | 목적 |
|------|------|------|
| 1 | **README.md** (이 파일) | 프레임워크 개요 파악 |
| 1.5 | [`soul.md`](soul.md) | 프로젝트 영혼 — DNA 유전 철학 |
...
> **부모-자식 문서 분리 패턴**: `AGENTICWORKFLOW-*.md`는 방법론/프레임워크(부모),
> `INVESTSCAN-*.md`는 도메인 고유 아키텍처(자식). 자식 문서만으로 InvestScan을 독립 운영할 수 있습니다.
