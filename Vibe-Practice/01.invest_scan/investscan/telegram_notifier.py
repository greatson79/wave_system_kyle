...
def send_document(
    ...
    """
    Send a document (e.g. the report PDF) via Telegram Bot API sendDocument.
    Dry-run mode: prints to stdout, returns True.
    ...
    if config.dry_run:
        print(f"\n[DRY-RUN] Telegram document would be sent to chat_id={config.chat_id}:")
    ...
    try:
        ...
        try:
            with open("logs/telegram_err.log", "a") as f:
                f.write(f"ERROR (sendDocument): {e}\n")
        except OSError:
            pass
        return False
