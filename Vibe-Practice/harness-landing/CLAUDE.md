# CLAUDE.md — harness-landing/

**Harness Engineering System — Landing-Page Edition** — Next.js/React **랜딩페이지** 자율 빌드용
3-에이전트 하네스. Planner → (Generator ↔ Evaluator) GAN 루프. 에이전트는 **파일로만** 통신한다.

> **`harness/`의 전용 복사본이다.** 범용 앱 빌더 `harness/`를 통째 복사해 랜딩페이지 전용으로
> 특화했다. 특화 지점: `prompts/*_system.md`(Next.js·섹션·CTA 중심), `criteria/landing_playbook.md`
> (신규 — CWV·반응형·anti-template 표준 번들), `criteria/evaluation_criteria.md`(가중치·차원 재정의),
> `config/harness_config.yaml`(max_sprints 5, craft 가중치 상향). 배관(`run_harness.py`·`_runtime`·
> `_context`·`schemas`)은 원본과 동일.

> **자기완결형 독립 프로젝트다.** 의존성은 `requirements.txt`에 전부 선언돼 있고, 산출물·생성
> 사이트·git은 모두 폴더 안에 머문다. 폴더를 통째로 다른 위치/별도 레포로 옮겨도 그대로 돌아간다.
...
| `prompts/*_system.md` | 시스템 프롬프트 (랜딩페이지·Next.js 특화) | 파일 경로·시그널 토큰·JSON 사이드카 지시 일관성 유지. 스택/포트(3000) 일관성 |
| `criteria/landing_playbook.md` | **랜딩페이지 표준 번들** (CWV·반응형·anti-template·a11y·SEO) | 3개 에이전트가 모두 읽음. `--bare`가 전역 web 규칙을 차단하므로 핵심을 여기 둠 |
| `criteria/evaluation_criteria.md` | 채점 루브릭 | `config`의 weights·thresholds와 차원·가중치 일치 유지 |
| `schemas/*.json` | JSON Schema | 변경 시 dry-run 스텁·에이전트 프롬프트 동기화 |
| `config/harness_config.yaml` | 튜너블 | `evaluation.weights` 합은 1.0 (로더가 강제). 현재 0.35/0.25/0.25/0.15 |
...
python run_harness.py "<랜딩페이지 아이디어>" --dry-run
python run_harness.py "<랜딩페이지 아이디어>"   # 실 빌드 (수시간·수달러 — 주의)
```

생성된 사이트는 `workspace/<run-id>/`에 Next.js 프로젝트로 빌드된다. 확인: `cd workspace/<run-id>
...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-04 | `harness/` 복사 후 랜딩페이지 특화 | 전체 | Next.js 랜딩페이지 전용 하네스 구축 |
