...
| 계층 | 이름 | 유형 | 수행 조건 |
|------|------|------|----------|
| **L0** | Anti-Skip Guard | 결정론적 | 항상 (L0d: config 정의 단계만) |
| **Pre-L1** | /simplify Quality Pass | 비차단(advisory) | 코드 단계 Step 9-17만 |
| **L1** | Verification Gate | 의미론적 | `Verification` 필드 있는 단계 |
| **L1.5** | pACS (Self-Rating) | 자기 평가 | pACS 활성 + Verification 통과 |
| **L2** | Adversarial Review (+Focus) | 적대적 검토 | `Review:` 필드 지정 단계 |

**L0: Anti-Skip Guard (결정론적)**
1. L0a-L0c: 산출물 파일이 SOT `outputs`에 기록 + 디스크 존재 + ≥ 100 bytes
2. L0d: 기대 구조 패턴 존재 확인 (`config/output-structure.yaml` 정의 — WARNING only, 캘리브레이션 중)

**Pre-L1: /simplify Quality Pass (비차단 — 코드 단계만)**
3. 코드 생성 단계(Step 9-17)에서 L1 진입 전 /simplify 실행
4. 기계적 코드 품질(중복, 효율, 가독성) 3축 자동 리뷰
5. 비차단 — 게이트가 아닌 품질 패스 (advisory)

**L1: Verification Gate (의미론적 — `Verification` 필드 있는 단계만)**
6. 산출물이 `Verification` 기준을 100% 달성했는지 에이전트가 자기 검증
7. 실패 기준 발견 시 재시도 전 **Abductive Diagnosis** 수행(P1 사전 증거 수집 → LLM 원인 분석 → P1 사후 검증) 후 진단 기반 재실행 (최대 10회 재시도, 무진전 시 Circuit Breaker 조기 에스컬레이션, ULW 시 15회)
8. `verification-logs/step-N-verify.md`에 기준별 PASS/FAIL + Evidence 기록. 진단 로그는 `diagnosis-logs/step-N-{gate}-{timestamp}.md`에 기록

**L1.5: pACS — predicted Agent Confidence Score (자기 신뢰 평가)**
9. Verification 통과 후, 에이전트가 F(Factual Grounding)/C(Completeness)/L(Logical Coherence) 3차원으로 자기 산출물을 채점
10. pACS = min(F, C, L) — 가장 약한 차원이 전체 신뢰도 결정
11. RED(< 50) → **Abductive Diagnosis** 수행 후 진단 기반 재작업, YELLOW(50-69) → 경고 후 진행, GREEN(≥ 70) → 통과
12. `pacs-logs/step-N-pacs.md`에 Pre-mortem 답변 + 점수 기록

**L2: Adversarial Review (Enhanced + Focus — `Review:` 필드 지정 단계만)**
13. `@reviewer`(코드/산출물 비판적 분석) 또는 `@fact-checker`(외부 사실 검증) Sub-agent가 독립적으로 적대적 검토
14. Focus Context 제공 시(`config/review-focus.yaml`), critical 영역에 2-3배 분석 깊이 할당
15. P1 검증(`validate_review.py`)으로 리뷰 품질 결정론적 보장
16. `review-logs/step-N-review.md`에 기록
17. 상세: `AGENTS.md §5.5`
...
- 재시도 진전 없음(pACS 3회 연속 ≤5점 개선) 시 Circuit Breaker가 조기 에스컬레이션 — 예산 낭비 방지
...
| 매 응답 후 | Stop hook이 retry record 누락 감지 | counter > 0인데 history 부재 시 경고 생성 (안전망) |
...
Autopilot과 ULW가 동시에 활성화되면 **ULW가 Autopilot을 강화**합니다: 품질 게이트 재시도 한도를 10→15회로 상향하고, Circuit Breaker(무진전 감지)는 ULW에서도 동일하게 동작합니다. Safety Hook 차단은 항상 존중합니다.
...
단계 실행 — 완전한 품질로 산출물 생성
  ↓
Anti-Skip Guard — L0a-L0c 파일 존재 + ≥ 100 bytes + L0d 구조 패턴 (WARNING)
  ↓
[Pre-L1 /simplify — 코드 단계만, 비차단]
  ↓
Verification Gate — 산출물을 각 기준 대비 자기 검증
  ├─ 모든 기준 PASS → verification-logs/step-N-verify.md 생성 → 진행
  └─ FAIL → 실패 부분만 재실행 (최대 10회, 무진전 시 Circuit Breaker 조기 에스컬레이션) → 초과 시 사용자 에스컬레이션
```
