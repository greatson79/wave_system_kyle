...
├── 자식 시스템 문서 (이 프로젝트 고유)
│   ├── README.md                              ← 프로젝트 개요
│   ├── SAAS-AUTOBUILDER-ARCHITECTURE.md       ← 도메인 아키텍처 (DAG, 에이전트, Hook, 품질 게이트)
│   └── SAAS-AUTOBUILDER-USER-MANUAL.md        ← 사용자 메뉴얼 ("시작" → 자동 실행)
│
...
│   ├── AGENTICWORKFLOW-USER-MANUAL.md         ← 부모 프레임워크 사용자 메뉴얼
...
│   │   ├── prd-analyst.md ... prd-writer-business.md (Phase 1 전용 12개)
...
│   │   ├── workflow-status.md       (/workflow-status — 진행 대시보드)
...
│   │   ├── _workflow_registry.py    (다형적 워크플로우 접근)
│   │   ├── orchestrator_actions.py  (8개 결정론적 서브커맨드 — 할루시네이션 봉쇄)
│   │   ├── sot_manager.py           (SOT 단일 쓰기 경로)
│   │   ├── quality_gate_runner.py   (4계층 품질 게이트 — L0→L1→L1.5→L2)
│   │   ├── context_guard.py         (통합 디스패처)
│   │   ├── _context_lib.py          (공유 라이브러리 — 파싱·생성·검증·압축)
│   │   ├── block_destructive_commands.py (위험 명령 차단, exit 2)
│   │   ├── output_secret_filter.py  (시크릿 탐지, 25+ 패턴)
│   │   ├── validate_*.py            (P1 결정론적 검증 스크립트 14개)
│   │   └── _test_*.py               (테스트 스위트 — 131+ 테스트)
...
├── translations/glossary.yaml       ← 번역 용어 사전 (EN-KO)
├── prompt/                          ← 연구 자료 + 워크플로우 정의 (90+ 파일)
└── coding-resource/                 ← 참조 PRD + 이론 기반
    └── PRD.md                       ← 입력 PRD (2,667줄, 161KB)
```

> **부모-자식 문서 분리**: `AGENTICWORKFLOW-*.md`는 부모 프레임워크(방법론)를, `SAAS-AUTOBUILDER-*.md`는 이 프로젝트 고유의 도메인 아키텍처를 기술한다.
...
| 사용자 요청 패턴 | 스킬 | 진입점 |
|----------------|------|--------|
| "시작", "start", "시작하자", "워크플로우 시작", "작업 시작", "실행" | `/start` | Smart Router → 실행 모드 선택 |
| "워크플로우 만들어줘", "자동화 파이프라인 설계" | `workflow-generator` | SKILL.md |
| "논문 스타일로 써줘", "학술적 글쓰기" | `doctoral-writing` | SKILL.md |

> **자연어 시작 라우팅**: 사용자가 "시작", "start", "시작하자", "워크플로우를 시작하자", "작업을 시작하자", "실행해줘", "go", "let's go", "begin" 등 **시작을 의미하는 자연어**를 입력하면, 반드시 `/start` 스킬을 호출하여 스마트 라우터 흐름을 시작한다. 워크플로우 설계(workflow-generator)와 혼동하지 않는다 — "시작"은 **실행**, "만들어줘"는 **설계**.
...
**4계층 품질 보장**: L0(Anti-Skip Guard) → L1(Verification Gate) → L1.5(pACS Self-Rating + Cross-Validation) → L2(Calibration). 상세: `docs/protocols/quality-gates.md`

**할루시네이션 봉쇄 (P1-P5)**: L1 문서 게이트가 구조적 검증(D1-D5)으로 auto-pass를 대체. 단계 진행 Guard(SP1-SP6)가 SOT 미업데이트/번역 누락/pACS 결정 불이행을 차단. pACS 교차 검증이 낙관적 자기평가를 탐지.
