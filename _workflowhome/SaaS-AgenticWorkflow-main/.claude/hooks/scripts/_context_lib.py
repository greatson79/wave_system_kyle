...
def validate_sot_schema(ap_state):
    ...
    if pacs is not None:
        if not isinstance(pacs, dict):
            ...
            if weak is not None and weak != "" and weak not in ("F", "C", "L"):
                warnings.append(
        ...
        try:
            ...
            if sot_file.endswith(".json"):
                ...
                try:
                    ...
                    continue
            if isinstance(data, dict):
                # Support both flat (legacy) and nested (workflow.outputs) SOT
...
def validate_translation_output(project_dir, step_number, source_path=None):
    ...
    Checks:
      ...
      T3: English source file exists (from SOT outputs or source_path)
    ...
    Args:
        project_dir: Project root directory path
        step_number: Step number (int) to validate
        source_path: Optional explicit English source path. When provided,
                     T3 uses this directly instead of SOT lookup.
                     Used by team steps where sub-outputs aren't in SOT
                     (CRITICAL-5 resolution).
    ...
    if source_path is not None:
        # Explicit source_path provided (e.g., team sub-output) — skip SOT lookup
        en_path = (
            source_path if os.path.isabs(source_path)
            else os.path.join(project_dir, source_path)
        )
    else:
        # Default: look up from SOT outputs
        outputs = _read_sot_outputs(project_dir)
        if outputs:
