...
def _build_recovery_output(source, latest_path, summary, sot_warning, snapshot_age, fallback_note="", project_dir=None, snapshot_content="", risk_data=None):
    ...
    if has_archive:
        ...
        if os.path.exists(ki_path):
            ...
            # CM-1: Proactive Design Decision surfacing
            # Surface recent design decisions for immediate context awareness
            design_decisions = _extract_recent_design_decisions(recent)
            if design_decisions:
                output_lines.append("")
                output_lines.append("■ 최근 설계 결정 (자동 표면화):")
                for dd in design_decisions[:3]:
                    output_lines.append(f"  - {dd}")

        if os.path.isdir(sessions_dir):
            output_lines.append(f"■ 세션 아카이브: {sessions_dir}")

    # CM-2: Workflow Phase awareness — detect active phase from SOT
    if project_dir:
        wf_phase_info = _detect_workflow_phase(project_dir)
        if wf_phase_info:
            output_lines.append("")
            output_lines.append(f"■ 워크플로우 Phase: {wf_phase_info}")
...
def _extract_recent_design_decisions(recent_sessions):
    """CM-1: Extract design decisions from recent Knowledge Archive sessions.

    Proactively surfaces key design decisions at SessionStart, enabling
    immediate context awareness without requiring manual Grep or Read.
    ...
    results = []
    for session in reversed(recent_sessions):
        decisions = session.get("design_decisions", [])
        if not isinstance(decisions, list):
            continue
        for d in decisions:
            if isinstance(d, str) and d.strip():
                # Skip low-signal intent markers
                if d.startswith("[intent]"):
                    continue
                results.append(d.strip()[:150])
        if len(results) >= 3:
            break
    return results[:3]


def _detect_workflow_phase(project_dir):
    """CM-2: Detect active workflow phase from SOT for context recovery.

    Reads state.yaml to determine whether Phase 1 (PRD) or Phase 2
    (full-stack development) is active. Returns a human-readable string
    for immediate context awareness at SessionStart.

    P1 Compliance: Deterministic read from SOT file.
    SOT Compliance: Read-only — never modifies state.yaml.
    Returns: info string or None if no SOT exists.
    """
    try:
        import yaml
    except ImportError:
    ...
    for sot_path in sot_paths(project_dir):
        ...
        try:
            ...
            # Detect phase from workflow name
            if "fullstack" in wf_name.lower() or "development" in wf_name.lower():
                phase = "Phase 2 (Full-Stack Development)"
            else:
                phase = "Phase 1 (PRD Generation)"

            return f"{phase} — Step {current_step}/{total_steps}, status: {status}"
        except Exception:
            continue
