...
# ---------------------------------------------------------------------------
# SOT direct write patterns — only sot_manager.py may write state.yaml
# GAP-C3: Complement validate_sot_write.py (Write tool) with Bash redirect blocking
# ---------------------------------------------------------------------------
SOT_WRITE_PATTERNS = [
    # Direct redirect to state.yaml/yml/json
    (
        re.compile(r">\s*\.claude/state\.(yaml|yml|json)"),
        "Direct write to SOT file (.claude/state.*) is blocked. "
        "Use sot_manager.py for all SOT mutations.",
    ),
    (
        re.compile(r">\s*\$CLAUDE_PROJECT_DIR/\.claude/state\.(yaml|yml|json)"),
        "Direct write to SOT file via $CLAUDE_PROJECT_DIR is blocked. "
        "Use sot_manager.py for all SOT mutations.",
    ),
    # tee to state files
    (
        re.compile(r"\btee\b.*\.claude/state\.(yaml|yml|json)"),
        "tee to SOT file (.claude/state.*) is blocked. "
        "Use sot_manager.py for all SOT mutations.",
    ),
]
...
def check_command(command: str) -> Optional[str]:
    ...
    # SOT direct write patterns: check entire command string
    for pattern, message in SOT_WRITE_PATTERNS:
        if pattern.search(command):
            return message
