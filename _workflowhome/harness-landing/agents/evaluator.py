...
from ._runtime import ensure_ok, run_claude, run_claude_resilient
...
the final line of the .md. Then stop."""
    ...
    ensure_ok(res, "evaluator:review_contract")
...
SPRINT_FAILED as final lines of the .md. Do NOT edit app source."""
    # resilient: live-browser QA can be transiently SIGKILL'd by an OS memory spike
    # (Chromium + dev server). Retry once on a signal-kill before giving up.
    res = run_claude_resilient(
        ctx,
    ...
    ctx.record_cost(res.cost_usd)
    ensure_ok(res, f"evaluator:qa:r{round}")
    ...
    # freshness guard: the report MUST be for this round. A stale report (same file from a
    # prior round, because this round's evaluator wrote nothing) would otherwise re-drive the
    # same verdict forever. This is the exact failure that silently spun a wasted run.
    if report.get("round") != round:
        raise RuntimeError(
            f"[evaluator:qa sprint {sprint}] expected a fresh QA report for round {round} "
            f"but found round {report.get('round')} — the evaluator produced no new report "
            f"this round (stale artifact). Stopping. Agent output head: {res.text[:300]!r}"
        )
